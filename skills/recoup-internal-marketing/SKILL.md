...
- `references/music-video.md` — **artist music video**: a real artist's released song cut to 9:16. Gate 0 (ask what they would make with no limits, and build THAT), casting invented characters instead of building a face guide, the rights gates that survive, the two-model motion split (OmniHuman for mouths, MiniMax H3 Max for everything else) and how to keep H3 from inventing props, word-timing sources, and the cost shape
