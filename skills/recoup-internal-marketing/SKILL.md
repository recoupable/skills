...
- `references/music-video.md` — **artist music video**: a real artist's released song cut to 9:16 with their real face and master. The five gates that each cost money on the reference build, the two-model motion split (OmniHuman for mouths, MiniMax H3 Max for everything else) and how to keep H3 from inventing props and signage, word-timing sources, and the bake-off-inclusive cost ledger
