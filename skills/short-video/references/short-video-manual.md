...
- A `song.mp3` for step 5. **Don't ask the user for a local file.** See `references/song-sourcing.md` for the fallback chain.
...
## Step 1: Pick a template (list **and** detail)
...
This call routinely takes 60–180s. **From a short-shell agent, use the async pipeline in `skills/short-video/SKILL.md` instead.** From a long-lived shell, output is an 8s clip. Step 5 loops it to song length.
...
SONG_PATH="$VIDEO_DIR/song.mp3"           # see references/song-sourcing.md
