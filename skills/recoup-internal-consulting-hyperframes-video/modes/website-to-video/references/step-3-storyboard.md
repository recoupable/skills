...
Pick from the inventory in capabilities.md (`capabilities.md`) and implementation patterns in techniques.md (`../../../engine/hyperframes-animation/techniques.md`). Examples of composable beats:
...
- **techniques.md (`../../../engine/hyperframes-animation/techniques.md`)** — 13 primitive animation techniques with code patterns. Pick for beats, these are starting points to adapt, not templates to copy.
- **text-effects.md (`../../../engine/hyperframes-animation/adapters/animate-text.md`)** — 24 named text animation effects from the separate `pixel-point/animate-text` skill. The reference page tells you how to load the upstream skill; the IDs are listed inline. Assign a specific effect ID to every headline, label, and copy element in every beat — not generic "fades in" descriptions.
...
**Global guardrails** — read video-composition.md (`../../../engine/hyperframes-creative/references/video-composition.md`) first. It defines the medium rules: density, color presence, scale, frame composition, and how design.md is brand truth not layout spec. Then apply these capture-specific additions:
...
**Fictional example:** a studio captures its product diagrams and illustrations but uses only a logo in the final video. The result loses the product's visual identity. Review the supplied assets before deciding what belongs in each scene.
...
When planning beats, decide which ones deserve an HTML-in-Canvas treatment vs. a standard GSAP animation. If you want it, name it in the storyboard — Step 5 will read `../../../engine/hyperframes-animation/adapters/html-in-canvas-patterns.md` for implementation. You don't need to specify the API details here.
...
```bash
# Resolve this mode through the installed skill registry; use its assets/sfx/manifest.json.
```
...
Every text element in this beat must name a specific effect from the catalog. The reference page is at `../../../engine/hyperframes-animation/adapters/animate-text.md` (or locate it with `<installed-mode>` (resolved from the installed skill registry)). It lists 24 effect IDs (from the separate `pixel-point/animate-text` skill); pick what fits the brand and this beat's mood — don't default to the same effect every beat.
