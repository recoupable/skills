...
Create `compositions/frames/`. Read `sub-agents/frame-worker.md` and `../../engine/hyperframes-core/references/subagent-dispatch.md`. Dispatch **one frame-worker per frame**, in parallel where possible (otherwise in waves). Each worker gets exactly one frame and this context:
...
| `references/planning.md` · `storyboard-format.md` (`references/storyboard-format.md`) | Step 3: pick the brand, fill each frame, write the plan |
