...
if (!heroInline && !HEROLESS) {
  if (dna.hero.setpiece === "detonation") {
    ...
    const CPM = JSON.parse(
      fs.readFileSync(requireCoverwordInputs().metricsPath, "utf8"),
    );
...
function requireCoverwordInputs() {
  ...
  if (!fontPath || !metricsPath) throw new Error(
    "The coverword theme requires an approved TTF in HYPERFRAMES_BRAND_FONT and matching glyph metrics in HYPERFRAMES_BRAND_METRICS. Demo fonts are not distributed. Choose the Recoup theme for bundled brand fonts.");
  return { fontPath, metricsPath };
}

function setpieceCoverword() {
  // CP2077 COVER-LETTERFORM slam, precision pass: the spoken apex word set in
  // the replica typeface of the official mark (an approved TTF supplied by the workspace
  ...
  const { fontPath, metricsPath } = requireCoverwordInputs();
