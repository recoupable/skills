...
> Read this file **only when** authoring a full scene from a pre-designed multi-phase choreography template. For atomic motion techniques, the rules index in GUIDE.md (`./GUIDE.md`) is the default starting point.
...
If no blueprint fits, **compose rules directly** from GUIDE.md (`./GUIDE.md`) — blueprints are starting points, not requirements.
