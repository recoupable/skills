...
- **Codex**: use the delegation tools actually available in the current session and follow its authorization rules. Include explicit workspace/output paths in each prompt and verify the returned artifacts. If delegation is unavailable, perform the roles serially; a shell child is not a way around a host restriction.
