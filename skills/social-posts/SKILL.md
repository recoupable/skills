...
Authenticate to the Recoup API the same way as the `recoup-api` skill: an `x-api-key` (`RECOUP_API_KEY`) or `Authorization: Bearer` (`RECOUP_ACCESS_TOKEN`) header. Never hardcode credentials.
