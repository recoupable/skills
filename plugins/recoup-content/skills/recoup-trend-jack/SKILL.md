...
- "Make a TikTok for Mari Vega" → a **format** request → router → `recoup-short-video`.
...
| --- | --- |
| Streams / followers milestone, playlist add, chart entry | `recoup-promo-graphic` (a "the numbers" graphic) |
| Sync / press / co-sign, or a story to tell | `recoup-brand-voice-caption` (+ a graphic) |
| A sound/format trend, "make it move" | `recoup-short-video` (let the song's audio drive the edit) |
