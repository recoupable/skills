---
name: recoup-content-video
description: Produce a finished 9:16 short-form music video (TikTok/Reel/Short) for an artist and their song. Use when the user says "make a video for [artist]", "create a TikTok for [artist]", "produce a Reel for [artist]", or types `/recoup-content-video`. Produces a complete, edited video — not clip selection; to find which 5–15s moment to clip, use recoup-song-hook. The default front door for content video.
...
This skill is **user-invoked** — the user types `/recoup-content-video <artist-name>` and the harness invokes it with `$ARGUMENTS` populated. It is self-contained: the default async path below runs without any other skill. For granular control (swapping a single stage), it points to the `short-video` skill in the Recoupable skills library.
...
- An artist record already exists for the requested artist on the user's first org. If not, run the `recoup-artist-create` skill (Recoupable skills library) first.
...
Use **`account_id`**, not `id` — `id` is the artist row's primary key, `account_id` is the underlying account that owns it. Passing the wrong one returns a 404 from `/api/content/create`. If no match, the artist record doesn't exist yet — run the `recoup-artist-create` skill first.
...
- The artist **doesn't exist yet**. Use the **`recoup-artist-create`** skill (Recoupable skills library) first.
