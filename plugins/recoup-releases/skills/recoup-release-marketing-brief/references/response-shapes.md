...
- **`tracks` / `track`** — track objects carry an `id` you pass to `/research/track` and `/research/track/playlists`. Per-song TikTok fields may or may not be present per track; if absent, report "no data" (never fabricate — see `recoup-tiktok-per-song`).
