...
8. **Normalize royalty statements.** Convert every statement into ledger rows
   ...
   participant shares, owner net, and source lineage. For supported CSV/TSV/XLSX
   exports, use `scripts/normalize-royalty-statement.py` (pass `--append` so each
   statement accumulates into one `royalty-ledger.csv` instead of overwriting).
   For text-based PDF statements, use `scripts/extract-pdf-statement.py`
   (experimental, verify-required, writes a separate review ledger). See
