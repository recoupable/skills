---
name: recoup-streaming-check
description: Checks an artist's streaming health and flags spikes or drops by reading current Recoup research API metrics and diffing them against the last check. Use when asked to "check streaming", "are streams spiking", "is anything blowing up", "streaming health check", "did streams jump this week", or as the post-release monitor armed by a release campaign. Identifies which playlist or release likely drove a move; refuses to fabricate numbers. Needs the artist name.
---
