...
1. Confirm `workflow_type` (`buy-side` / `recoup-seller-prep` / `financing`)
   ...
   otherwise infer from the user's message.
...
1. `workpapers/nps-bridge.json` and `workpapers/nls-bridge.json` —
   ...
   them directly from recoup-royalty-audit findings.
...
Use the `recoup-catalog-dashboard` skill. **The agent owns this artifact.**
...
- **Demo data permissible.** If the user explicitly says "this is a
  demo, make up a data room" (or runs `/recoup-catalog-demo`), generate
...
- `/recoup-catalog-kickoff` — Phase 1 only.
- `/recoup-catalog-ingest` — Phase 2 only.
- `/recoup-catalog-analyze` — Phases 3–4 only.
