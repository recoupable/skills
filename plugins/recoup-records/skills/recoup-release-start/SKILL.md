...
Run **`recoup-release-brief`** for this artist + release. It pulls
...
1. **`recoup-artist-playlists`** → which playlists to target (gap vs. peers).
2. **`recoup-artist-outreach`** → the curator behind each top target + a drafted pitch (draft only — never send).

If the artist's audience picture is thin, also run
**`recoup-artist-audience`** to ground the targeting.
...
- **Copy that needs the house voice** → run it through `recoup-content-caption`.
...
| sync-focused | lead with `recoup-song-pitch-kit` placements |
| catalog-reissue | anniversary hooks + existing-audience reactivation, lighter discovery |

## Power-user escape hatches

Run a single stage directly: `recoup-release-brief` (creative),
