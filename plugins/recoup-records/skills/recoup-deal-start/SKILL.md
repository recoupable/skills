...
description: One-command end-to-end music catalog deal review. Use whenever the user types `/recoup-deal-start`, says "review this catalog", "analyze this music catalog deal", "do diligence on this deal", "underwrite this catalog", or asks for a full catalog acquisition / seller-prep / financing / post-close review (for just the valuation, use recoup-deal-value). Scaffolds the workspace, normalizes the data room, runs analysis, builds an agent-authored DASHBOARD.html, validates it, and drafts the IC memo without stopping for permission between phases. The default front door for the recoup-deals.
...
2. **Resolve column maps first (per file), then build the ledger in one append
   loop.** Doing map-resolution before the ledger build keeps the build a single
   clean pass — no double-counting from re-running a corrected file.

   a. For each file the manifest tags with a `likely_provider`, do a discovery
      run to a throwaway part to read its `status`:
      `python3 scripts/normalize-royalty-statement.py --provider <key> --input <path> --output deals/{deal-id}/normalized/parts/<source-file>.csv`.

      **If it returns `status: "partial"`, do not stop and do not ask the user.**
      Auto-recover with:
      ...
      If `auto-column-map` returns `status: "ok"`, that file will use
      `--column-map workpapers/column-maps/<source-file>.json` in the build. If it
      returns `status: "needs_review"`, log the file in
      `findings/manual-review-queue.md` with the missing critical fields and
      exclude it from the build.

   b. Build the single ledger. Delete it first so re-runs start clean, then loop
      every qualifying file with `--append` (and its column map if one was
      resolved). Without `--append` each call **overwrites** and you silently keep
      only the last provider:

      ```bash
      LEDGER=deals/{deal-id}/normalized/royalty-ledger.csv
      rm -f "$LEDGER"
      # for each qualifying file:
      python3 scripts/normalize-royalty-statement.py --provider <key> --input <path> \
        --output "$LEDGER" --append [--column-map workpapers/column-maps/<source-file>.json]
      ```

      Confirm `ledger_total_rows` grows with each statement.
...
1. `workpapers/nps-bridge.json` and `workpapers/nls-bridge.json` —
   ...
   them directly from the royalty audit (the `recoup-deal-value`
...
| Workflow | Analysis emphasis (`recoup-deal-value` reference) | Report package (`recoup-deal-report`) |
| --- | --- | --- |
...
- `/recoup-deal-value` — Phases 3–4 only (rights, royalty, valuation diligence).
