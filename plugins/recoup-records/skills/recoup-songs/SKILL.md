---
name: recoup-songs
description: Understand and work with ONE song from its audio, using Recoup's audio language model (Music Flamingo). Modes — analyze (full report, catalog metadata BPM/key/genre/mood, lyric sheet, or mix critique), hook (find the most clip-worthy 5–15s moment to lead short-form with), and pitch (playlist/editorial pitch + music-supervisor sync brief). Use for "analyze this song", "what key/BPM/genre is this", "tag this track", "transcribe the lyrics", "critique the mix", "find the hook", "best 15 seconds to clip", "write a playlist pitch", or "make a sync brief". Needs a public audio URL. For writing lyrics from scratch (no audio) use recoup-songswriting; for catalog-wide playlist strategy (no audio) use recoup-research (playlists).
---
