...
description: >
  ...
  {release}", "what should I plan for {release}", "pre-release brief for {release}".
  ...
  this one is the CREATIVE BRIEF (a plan, not finished assets — to actually produce
  the assets, use recoup-content). Refuses to invent release details (sound, lyrics,
...
For ongoing weekly artist tracking, use `recoup-artist-brief`. For one-shot
...
- Approved playlist pitch emails (use `recoup-artist-playlists` for that)
...
---
*Generated {ISO timestamp}. Re-run with `/recoup-release-brief {release-slug}`.*
```
...
- `recoup-artist-playlists` — for the actual playlist pitch emails
- `recoup-artist-audience` — for deeper geographic / demo analysis
- `recoup-artist-brief` — for ongoing tracking after release
