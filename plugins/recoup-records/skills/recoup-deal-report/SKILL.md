---
name: recoup-deal-report
description: Use when the user wants to assemble a deal's IC memo / seller cleanup report / financing pack and turn the dashboard, memos, and workpapers into a single shareable, emailable PDF report. Trigger on "write the IC memo", "draft the memo", "build the PDF report", "export the report", "make a PDF I can email", "share this deal", "PDF version of the dashboard", "buyer-ready PDF", "IC-ready PDF", "financing pack PDF", or any request to package a catalog deal for distribution.
...
- After `/recoup-deal-start` finishes, when the user wants to
...
## Assemble the memo first (this skill owns the written narrative)
...
| `post-close-admin` | `memos/post-close-admin-plan.md` | derive from the `recoup-deal-value` skill's post-close-admin playbook |

Rules for memo assembly:

1. **Copy the matching template** into `memos/` if the memo does not
   exist yet, then fill every section. Pull every material number from
   the workpapers (`workpapers/*.json`) — never from prose memory.
2. **Lead with the decision.** Value bracket, normalized NPS / NLS, top
   blocker, and the recommendation come first. Detail follows.
3. **Every `$`-figure obeys the trust contract above** — it matches a
   workpaper value within 5%, or it cites an `evidence-ledger.json`
   entry. No unsupported numbers.
4. **List open findings honestly.** Material findings from
   `findings/findings.json` are closed, accepted, or shown as open —
   never silently dropped.
5. If the memo already exists and is current, refresh only the sections
   whose underlying workpapers changed this session.

The PDF is the same story formatted for distribution. Once the memo is
ready, convert as below.
