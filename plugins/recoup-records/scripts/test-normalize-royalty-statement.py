...
class NormalizeRoyaltyStatementTest(unittest.TestCase):
    ...
    def test_append_accumulates_multiple_providers_without_overwrite(self) -> None:
        """Two providers written to one ledger with --append must both survive,
        keep a single header, and carry unique ledger_line_ids (regression for
        the overwrite bug where each call clobbered the previous provider)."""
        ascap_rows = [
            {"Work Title": "Hook A", "ISWC": "T1111111110", "Performance Start": "2025-01-01", "Performance End": "2025-03-31", "Gross Amount": "100.00", "Publisher Share": "90.00", "Currency": "USD"},
            {"Work Title": "Hook B", "ISWC": "T2222222220", "Performance Start": "2025-01-01", "Performance End": "2025-03-31", "Gross Amount": "200.00", "Publisher Share": "180.00", "Currency": "USD"},
        ]
        dist_rows = [
            {"Track Title": "Night Drive", "ISRC": "USRC17607839", "Sales Month": "2025-02", "Store": "Spotify", "Gross": "500.00", "Net": "425.00", "Currency": "USD"},
        ]
        with tempfile.TemporaryDirectory() as directory:
            ledger = Path(directory) / "royalty-ledger.csv"
            ascap_src = Path(directory) / "ascap.csv"
            dist_src = Path(directory) / "distributor.csv"
            write_csv(ascap_src, ascap_rows)
            write_csv(dist_src, dist_rows)

            def run(provider: str, src: Path) -> dict:
                result = subprocess.run(
                    [sys.executable, str(SCRIPT), "--provider", provider, "--input", str(src), "--output", str(ledger), "--append"],
                    text=True, capture_output=True,
                )
                self.assertEqual(result.returncode, 0, result.stderr)
                return json.loads(result.stdout)

            first = run("ascap", ascap_src)
            self.assertFalse(first["appended"], "first call should write fresh (file absent)")
            self.assertEqual(first["ledger_total_rows"], 2)

            second = run("distributor", dist_src)
            self.assertTrue(second["appended"], "second call should append to existing ledger")
            self.assertEqual(second["ledger_total_rows"], 3)

            ledger_rows = read_csv(ledger)
            self.assertEqual(len(ledger_rows), 3, "all three rows must survive both providers")
            providers = {row["provider"] for row in ledger_rows}
            self.assertEqual(providers, {"ASCAP", "Distributor"})
            ids = [row["ledger_line_id"] for row in ledger_rows]
            self.assertEqual(len(ids), len(set(ids)), f"ledger_line_id collision: {ids}")
            # exactly one header line in the raw file
            header_count = ledger.read_text(encoding="utf-8").count("ledger_line_id,catalog_asset_id")
            self.assertEqual(header_count, 1, "ledger must contain exactly one header row")
